...
// Shows all orders for logged-in user with 4-status progress tracker.
...
$allowed_filters = ['all', 'in_process', 'ready_for_delivery', 'on_delivery', 'delivered'];
...
if ($filter === 'all') {
    ...
    $stmt = $pdo->prepare("SELECT * FROM orders WHERE user_id = ? AND order_status = ? ORDER BY created_at DESC");
    $stmt->execute([$user_id, $filter]);
}
$orders = $stmt->fetchAll();

$count_stmt = $pdo->prepare("SELECT order_status, COUNT(*) as cnt FROM orders WHERE user_id = ? GROUP BY order_status");
$count_stmt->execute([$user_id]);
$counts = ['all' => 0, 'in_process' => 0, 'ready_for_delivery' => 0, 'on_delivery' => 0, 'delivered' => 0];
...
$spent_stmt = $pdo->prepare("SELECT COALESCE(SUM(total_amount),0) FROM orders WHERE user_id = ? AND order_status = 'delivered'");
...
<div class="page-wrapper">
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            <div class="stat-val"><?= $counts['in_process'] + $counts['ready_for_delivery'] + $counts['on_delivery'] ?></div>
    ...
    <div class="tabs">
        <?php foreach ($tab_labels as $key => $label): ?>
            <a href="my_orders.php?filter=<?= $key ?>"
    ...
    <?php if (empty($orders)): ?>
        ...
        <div class="card">
            <div class="empty-state">
                ...
                <p>
                    <?php if ($filter !== 'all'): ?>
                        No orders with status "<?= get_status_label($filter) ?>".
                ...
                <div class="order-items-box">
                    ...
                    <?php foreach ($order_items as $di): ?>
                        <div class="order-item-row">
                            ...
                            <span class="order-item-price">Rs <?= number_format($di['price'] * $di['quantity'], 0) ?></span>
                        </div>
                    <?php endforeach; ?>
                </div>
            <?php endif; ?>
