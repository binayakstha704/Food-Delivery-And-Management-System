...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($field_errors['general'])) {
    ...
    if (empty($field_errors)) {
        ...
        if (!$user) {
            // Unknown email — log failed attempt + user log
            ...
            // Inactive account
            log_failed_attempt($conn, $ip_address);
            log_user_event($conn, 'login_failed', $ip_address, "Login failed: inactive account for '{$email}'", (int)$user['user_id']);
            $field_errors['general'] = 'This account is inactive.';

        } elseif (!in_array($user['role'], ['customer', 'chef', 'staff'], true)) {
            // Invalid role
            log_failed_attempt($conn, $ip_address);
            log_user_event($conn, 'login_failed', $ip_address, "Login failed: invalid role '{$user['role']}' for '{$email}'", (int)$user['user_id']);
            ...
            // ── Credentials valid — clear rate limit ──────────
            clear_failed_attempts($conn, $ip_address);

            // Chef and staff must pass the role secret check before full login
            if (in_array($user['role'], ['chef', 'staff'], true)) {
                ...
                // We log success only after role secret is confirmed (in verify_role_secret.php).
                // Here we just note that credentials were accepted.
                log_user_event($conn, 'login_success', $ip_address, "Credentials accepted for {$user['role']} '{$email}' — pending role secret", (int)$user['user_id']);
                session_write_close();
                header('Location: verify_role_secret.php');
                exit;
            }

            // Unverified email — send OTP before letting them in
            if (empty($user['email_verified_at'])) {
                ...
                // ── Full login success ────────────────────────
                // Log the event BEFORE login_user() calls session_write_close()
                log_user_event($conn, 'login_success', $ip_address, "Successful login as {$user['role']}: {$user['full_name']} ({$email})", (int)$user['user_id']);
                login_user($user);
                redirect_user_by_role($user['role']);
            }
        }

        // Re-check rate limit: if THIS attempt pushed the IP over the threshold,
        // show the lockout message immediately rather than individual field errors.
        if (!empty($field_errors)) {
            $rate_status = is_rate_limited($conn, $ip_address);
            if ($rate_status['blocked']) {
                $field_errors = ['general' => "Too many failed login attempts. Please try again in {$rate_status['remaining']} minute(s)."];
...
<div class="container">
    <div class="login-box">
        ...
        <?php endif; ?>
        <?php if (isset($_GET['reason']) && $_GET['reason'] === 'security'): ?>
            <div class="alert error">🔒 Your session was reset for security reasons. Please log in again.</div>
        <?php endif; ?>
        <?php if (isset($_GET['registered'])): ?>
            <div class="alert success">✅ Email verified successfully. Please log in.</div>
        <?php endif; ?>
        <?php if (isset($_GET['pw_reset'])): ?>
            <div class="alert success">✅ Password reset successfully. Please log in with your new password.</div>
        <?php endif; ?>
        <?php if (!empty($_SESSION['login_error'])): ?>
            <div class="alert error">⚠️ <?= htmlspecialchars($_SESSION['login_error'], ENT_QUOTES, 'UTF-8') ?></div>
        ...
        <form method="POST" class="login-form" id="loginForm">
            ...
            <div class="input-group <?= isset($field_errors['email']) ? 'has-error' : '' ?>">
                ...
                <div class="input-wrapper">
                    ...
                    <input
                        ...
                        required
                        <?= $rate_status['blocked'] ? 'disabled' : '' ?>
                    >
                </div>
                <?php if (isset($field_errors['email'])): ?>
                    <span class="field-error">⚠️ <?= htmlspecialchars($field_errors['email'], ENT_QUOTES, 'UTF-8') ?></span>
                <?php endif; ?>
            </div>
            ...
            <div class="input-group <?= isset($field_errors['password']) ? 'has-error' : '' ?>">
                ...
                <?php if (isset($field_errors['password'])): ?>
                    <span class="field-error">⚠️ <?= htmlspecialchars($field_errors['password'], ENT_QUOTES, 'UTF-8') ?></span>
                <?php endif; ?>
                <div style="text-align:right;margin-top:6px;">
                    <a href="forgot_password.php" style="font-size:0.82rem;color:#22c55e;text-decoration:none;">Forgot password?</a>
                </div>
            </div>
            ...
            <button
                ...
                <?= $rate_status['blocked'] ? 'disabled' : '' ?>
            >
                <?= $rate_status['blocked'] ? '🔒 Account Locked' : 'Login' ?>
            </button>
        </form>
