...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_item'])) {
    ...
    if ($item_id && $item_id > 0) {
        ...
        $ok = $stmt->execute();
        $stmt->close();
        $_SESSION['_toast'] = $ok ? ['type'=>'success','text'=>'Menu item deleted successfully.'] : ['type'=>'danger','text'=>'Failed to delete menu item.'];
    } else {
        $_SESSION['_toast'] = ['type'=>'danger','text'=>'Invalid menu item ID.'];
    }
    session_write_close();
    header('Location: chef-menu.php');
    exit;
...
$category_options_res = $conn->query('SELECT category_id, name FROM categories WHERE is_available = 1 ORDER BY name');
$category_options = $category_options_res ? $category_options_res->fetch_all(MYSQLI_ASSOC) : [];
...
$menu_items_arr = $menu_items ? $menu_items->fetch_all(MYSQLI_ASSOC) : [];
?>
<!DOCTYPE html>
<html lang="en">
<head>
        ...
        <div class="content">
            ...
            <section class="ops-panel">
                ...
                <form method="POST" class="chef-form-grid">
                    ...
                    <div><label>Category *</label><select name="category_id" required><option value="">Select Category</option><?php foreach ($category_options as $cat): $selected = $edit_item && (int)$edit_item['category_id'] === (int)$cat['category_id']; ?><option value="<?php echo (int)$cat['category_id']; ?>" <?php echo $selected ? 'selected' : ''; ?>><?php echo h($cat['name']); ?></option><?php endforeach; ?></select><?php if (isset($errors['category_id'])): ?><span class="error-message"><?php echo h($errors['category_id']); ?></span><?php endif; ?></div>
            ...
            <section class="ops-panel"><div class="ops-panel-heading"><div><p class="ops-eyebrow">Menu List</p><h2>Existing Menu Items</h2><p>Edit availability, price, rating, and descriptions.</p></div></div>
                <div class="ops-table-wrap"><table class="chef-table"><tr><th>ID</th><th>Category</th><th>Name</th><th>Description</th><th>Price</th><th>Rating</th><th>Available</th><th>Actions</th></tr><?php if (!empty($menu_items_arr)): $i=1; foreach ($menu_items_arr as $item): ?><tr><td><?php echo $i++; ?></td><td><?php echo h($item['category_name']); ?></td><td><?php echo h($item['name']); ?></td><td><?php echo h($item['description']); ?></td><td>Rs. <?php echo number_format((float)$item['price'], 2); ?></td><td><?php echo number_format((float)$item['rating'], 1); ?></td><td><?php echo ((int)$item['is_available'] === 1) ? 'Yes' : 'No'; ?></td><td><div class="action-links"><a class="ops-btn ops-btn-ghost" href="chef-menu.php?edit_id=<?php echo (int)$item['item_id']; ?>">Edit</a><form method="POST" style="display:inline" data-hc-confirm="Delete this menu item?" data-hc-confirm-icon="🗑️" data-hc-confirm-type="danger" data-hc-confirm-ok="Delete"><input type="hidden" name="item_id" value="<?php echo (int)$item['item_id']; ?>"><button type="submit" name="delete_item" class="ops-btn ops-btn-warning">Delete</button></form></div></td></tr><?php endforeach; else: ?><tr><td colspan="8">No menu items found.</td></tr><?php endif; ?></table></div>
            </section>
        </div>
    </div>
</div>
