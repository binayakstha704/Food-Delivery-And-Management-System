...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_update'])) {
    ...
    // Always generate a fresh UUID on each sync so every eSewa attempt is unique.
...
// eSewa requires alphanumeric + hyphen only, and the UUID must be unique per attempt.
// We always generate a fresh UUID on every full page load so that:
//   • going back from eSewa and clicking Pay again never reuses the previous UUID
//     (which eSewa rejects with "Duplicate transaction UUID")
//   • bfcache restores are handled by the pageshow reload below
$transaction_uuid = hc_generate_payment_uuid();
...
<script src="../assets/js/theme.js"></script>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        html[data-theme="light"] .card { background: #ffffff !important; border: 1px solid #e0e0e0 !important; box-shadow: 0 2px 12px rgba(0,0,0,0.06) !important; }
        html[data-theme="light"] .card-title { color: rgba(77,184,72,0.85) !important; }
        html[data-theme="light"] .order-item-name { color: #1c1c1c !important; }
        html[data-theme="light"] .order-item-qty { color: #666666 !important; }
        html[data-theme="light"] .summary-row { color: #444444 !important; border-bottom-color: rgba(0,0,0,0.05) !important; }
        html[data-theme="light"] .summary-row.grand { color: #4db848 !important; }
        html[data-theme="light"] .method-label { background: #f7f7f7 !important; border-color: #e0e0e0 !important; }
        html[data-theme="light"] .method-name { color: #1c1c1c !important; }
        html[data-theme="light"] .method-sub { color: #666666 !important; }
        html[data-theme="light"] label.field-label { color: #444444 !important; }
        html[data-theme="light"] .delivery-note { color: #666666 !important; }
        html[data-theme="light"] textarea { background: #f5f5f5 !important; border-color: #d0d0d0 !important; color: #1c1c1c !important; }
        html[data-theme="light"] textarea::placeholder { color: #999999 !important; }
        html[data-theme="light"] .location-select { background-color: #f5f5f5 !important; border-color: #d0d0d0 !important; color: #1c1c1c !important; }
        html[data-theme="light"] .location-select option, html[data-theme="light"] .location-select optgroup { background: #ffffff !important; color: #1c1c1c !important; }
    </style>
</head>
<body>
<div class="layout">
    ...
    <div class="main">
        ...
        <div class="content">
            ...
            <?php
            // eSewa proxy error — displayed when esewa_proxy.php bounces back
            $esewa_proxy_err = $_SESSION['_esewa_error'] ?? null;
            unset($_SESSION['_esewa_error']);
            if ($esewa_proxy_err || isset($_GET['esewa_err'])):
            ?>
            <div class="alert error" style="display:flex;gap:10px;align-items:flex-start;">
                <span style="font-size:1.3em;">🔌</span>
                <div>
                    <strong>eSewa Unavailable</strong><br>
                    <?= $esewa_proxy_err
                        ? htmlspecialchars($esewa_proxy_err, ENT_QUOTES, 'UTF-8')
                        : 'eSewa\'s payment server is temporarily unavailable. Please try again in a few minutes, or choose Cash on Delivery.' ?>
                </div>
            </div>
            <?php endif; ?>
            ...
            <!-- eSewa hidden form — posts to our proxy which forwards to eSewa -->
            <form id="esewa-form" action="esewa_proxy.php" method="POST">
    ...
    if (method === 'esewa') {
        ...
        pending
            ...
            .catch(() => {
                ...
                hcAlert('Unable to prepare eSewa payment. Please try again.', {icon:'❌', type:'danger'});
            });
        return;
    }
}
