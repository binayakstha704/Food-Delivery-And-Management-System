...
// Always restrict to only login/session event types — never show order events here
if ($ev_type !== '' && in_array($ev_type, $allowed_events, true)) {
    $ev_where[]  = 'ul.event_type = ?';
    $ev_params[] = $ev_type;
    $ev_types   .= 's';
} else {
    $placeholders = implode(',', array_fill(0, count($allowed_events), '?'));
    $ev_where[]   = "ul.event_type IN ($placeholders)";
    foreach ($allowed_events as $ae) { $ev_params[] = $ae; }
    $ev_types    .= str_repeat('s', count($allowed_events));
}
if ($ev_date !== '') {
...
<head>
    ...
    <style>
        ...
        html[data-theme="light"] .export-btn { color:#333333 !important; border-color:rgba(0,0,0,0.15) !important; background:rgba(0,0,0,0.04) !important; }
        html[data-theme="light"] .export-btn:hover { background:rgba(0,0,0,0.08) !important; color:#000 !important; }
            ...
            fetch('user-logs.php?live_poll=1&tab=events&since_id=' + evLastId, { cache: 'no-store' })
                ...
                .then(function(data){
                    ...
                    data.rows.forEach(function(row){
                        ...
                        // Prepend so newest events appear at the top (matching DESC sort)
                        evTbody.insertBefore(buildEvRow(row, '★'), evTbody.firstChild);
                        evNextSn++;
                        evTotal++;
                    });
