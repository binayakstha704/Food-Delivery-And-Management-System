...
if ($_SERVER['REQUEST_METHOD'] === 'POST'
    ...
    try {
        ...
        $refund_msg = ($c_order['payment_method'] === 'esewa')
            ? "Note: Since payment was made via eSewa, a refund is not available. Please contact canteen staff if you have concerns."
...
<head>
    ...
    <style>
        ...
        html[data-theme="light"] .order-delivery-location { background:rgba(77,184,72,0.06) !important; border-color:rgba(77,184,72,0.2) !important; }
        html[data-theme="light"] .dloc-label { color:#2a7a26 !important; }
        html[data-theme="light"] .dloc-name { color:#1c1c1c !important; }
        html[data-theme="light"] .dloc-block { color:#555555 !important; }
        html[data-theme="light"] .dloc-empty { color:#888888 !important; }
    </style>
</head>
<body>
...
<div class="page-wrapper">
    ...
    <div class="refund-notice">
        <div class="rn-title">⚠️ eSewa Payment — No Refund Available</div>
        <div class="rn-body">
            Your order has been cancelled. Since payment of <strong>Rs <?= number_format((float)$cancel_amount, 0) ?></strong> was made via eSewa,
            <strong>a refund is not available</strong>. Please contact canteen staff if you have concerns.
        </div>
    </div>
        ...
        ?>
            <div class="card order-card">
                ...
                <div class="order-delivery-location" style="margin-top:10px;padding:10px 12px;background:rgba(77,184,72,0.07);border:1px solid rgba(77,184,72,0.15);border-radius:8px;font-size:13px;">
                    <span style="font-size:11px;display:block;margin-bottom:3px;" class="dloc-label">📍 DELIVERY LOCATION</span>
                    <?php if (!empty($order['delivery_location_name'])): ?>
                        <strong class="dloc-name"><?= h($order['delivery_location_name']) ?></strong>
                        <span class="dloc-block" style="font-size:12px;"> — <?= h($order['delivery_block_name'] ?? '') ?></span>
                    <?php else: ?>
                        <span class="dloc-empty" style="font-style:italic;">Not specified</span>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
...
<div class="cancel-modal-overlay" id="cancelModalOverlay">
    <div class="cancel-modal">
        ...
        <div class="cancel-modal-actions" id="cancelModalCloseBtn" style="display:none;">
            <button type="button" class="btn-modal-dismiss" onclick="closeCancelModal()">Close</button>
        </div>
    </div>
</div>
...
function openCancelModal(orderId, payMethod, amount) {
    document.getElementById('cancelOrderForm').style.display = '';
    document.getElementById('cancelModalCloseBtn').style.display = 'none';
    ...
    if (payMethod === 'esewa') {
        policy.innerHTML =
            '<div class="policy-title">\u26A0\uFE0F eSewa Payment \u2014 No Refund Available</div>'
            + '<div class="policy-body">Your order will be cancelled, but since payment was already processed via eSewa, '
            + '<strong>a refund is not available</strong>. Please contact canteen staff if you have concerns.</div>';
    } else {
        policy.innerHTML =
