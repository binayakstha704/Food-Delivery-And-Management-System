    ...
    if (empty($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
        ...
        if ($submitted === '') {
            ...
            if (!$result['ok']) {
                ...
                // Log failed OTP attempts for login purpose so they appear in user-logs
                if ($purpose === 'login') {
                    $ip = $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0';
                    log_user_event($conn, 'login_failed', $ip, "Login failed: wrong OTP code entered for '{$otp_email}'", $user_id);
                }
            } else {
                unset($_SESSION['otp_pending']);
                ...
                if ($purpose === 'login') {
                    ...
                    log_user_event($conn, 'login_success', $ip, "Login successful (2-step) for '{$user['email']}'", (int)$user['user_id']);
