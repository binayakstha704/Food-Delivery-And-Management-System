...
<style>
/* ── Staff Order History: Delivery Location Card ── */
.sh-delivery-loc {
    display: flex;
    align-items: center;
    gap: 10px;
...
.sh-loc-icon { font-size: 15px; flex-shrink: 0; }
.sh-loc-details { display: flex; align-items: baseline; gap: 6px; flex-wrap: wrap; min-width: 0; }
.sh-loc-name {
    font-size: 13px;
    font-weight: 700;
    color: #ffffff;
...
html[data-theme="light"] .sh-loc-name  { color: #1a1a1a !important; }
html[data-theme="light"] .sh-loc-block { color: #666 !important; }
html[data-theme="light"] .sh-mode-takeaway    { color: #3a5fcc; background: rgba(99,150,255,0.1); border-color: rgba(99,150,255,0.3); }
html[data-theme="light"] .sh-mode-unspecified { color: #888; background: rgba(0,0,0,0.04); border-color: rgba(0,0,0,0.1); }
html[data-theme="light"] .sh-items-block { background: rgba(0,0,0,0.03); border-color: rgba(0,0,0,0.08); }
html[data-theme="light"] .sh-items-summary { color: #1a1a1a !important; }
</style>
</head>
    ...
    <div class="main">
        ...
        <div class="content">
            ...
            <section class="ops-stat-grid" aria-label="Paid history summary">
                ...
                <article class="ops-stat-card ops-stat-purple"><div class="ops-stat-icon">🖌️</div><div><span>Cleared From View</span><strong><?php echo (int)$summary['hidden']; ?></strong></div></article>
            ...
            <section class="ops-panel">
                <div class="ops-panel-heading">
                    ...
                    <form method="POST" data-hc-confirm="Hide all visible paid delivered orders from staff history? This does not delete database records." data-hc-confirm-icon="📋" data-hc-confirm-ok="Hide Orders">
                        <input type="hidden" name="clear_paid_history" value="1">
                        <button type="submit" class="ops-btn ops-btn-warning">🧹 Clear Paid History</button>
                    </form>
                </div>
                            ...
                            <article class="ops-order-card staff-order-card">
                                ...
                                <?php if (($order['delivery_mode'] ?? 'delivery') === 'delivery'): ?>
                                    <?php if (!empty($order['delivery_location_name'])): ?>
                                        <div class="delivery-location-card sh-delivery-loc">
                                            <span class="sh-loc-icon">📍</span>
                                            <span class="sh-loc-details">
                                                <strong class="sh-loc-name"><?php echo h($order['delivery_location_name']); ?></strong>
                                                <em class="sh-loc-block"><?php echo h($order['delivery_block_name'] ?? ''); ?></em>
                                            </span>
                                        </div>
                                    <?php else: ?>
                                        <div class="sh-mode-pill sh-mode-unspecified">📍 Delivery location not specified</div>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <div class="sh-mode-pill sh-mode-takeaway">🥡 Takeaway / Pickup</div>
                                <?php endif; ?>
                                <div class="ops-items-block sh-items-block">
                                    <div class="ops-items-title sh-items-title">Items</div>
                                    <p class="ops-items-summary sh-items-summary"><?php echo h($order['items_summary'] ?? 'No items found'); ?></p>
                                </div>
                            </article>
                        <?php endforeach; ?>
                    </div>
